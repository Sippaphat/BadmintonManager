...
function updateMatch(allPlayers, teamAIds, teamBIds, teamAWon) {
  ...
  const A = Array.from(teamAIds)
    .map((id) => playersMap[id])
    .filter(Boolean);
  const B = Array.from(teamBIds)
    .map((id) => playersMap[id])
    .filter(Boolean);
  ...
  const eloA =
    A.reduce((sum, p) => sum + (p.elo !== undefined ? p.elo : 1500), 0) /
    A.length;
  const eloB =
    B.reduce((sum, p) => sum + (p.elo !== undefined ? p.elo : 1500), 0) /
    B.length;
    ...
    const updatedPlayers = players.map((p) => {
      if (p.id === playerId) {
        return {
          ...p,
          isPlaying: false,
          playCount: Math.max(0, p.playCount - 1),
        };
      }
      return p;
    });
    ...
    const updatedCourts = courts.map((c) => {
      if (c.id === courtId) {
        ...
        if (
          isDoubles &&
          c.serving === team &&
          c.players.length === requiredPlayers
        ) {
    ...
    return (
      ...
      >
        <div className="courtHeader">
          <button onClick={() => setShowCalendar(false)} className="resetBtn">
            {t("cancelAction")}
          </button>
          <h2 style={{ fontSize: 18, margin: 0, color: COLORS.primary }}>
            {t("calendar")}
          </h2>
        ...
        <div style={{ marginTop: 20 }}>
          ...
          >
            <h3 style={{ fontSize: 16, margin: 0, color: COLORS.text }}>
              {t("calendar")} List
            </h3>
            {selectedGroup && (
              <button
            ...
            return (
              ...
              >
                ...
                >
                  ...
                  <button
                    onClick={() => handleDeleteSchedule(ev._id)}
                    style={{
                      border: "none",
                      background: "none",
                      color: COLORS.red,
                ...
                >
                  ...
                  <a
                    href={generateGoogleCalendarLink(ev)}
                    target="_blank"
                    rel="noreferrer"
                    style={{
                      fontSize: 12,
                  ...
                  <a
                    href={generateOutlookCalendarLink(ev)}
                    target="_blank"
                    rel="noreferrer"
                    style={{
                      fontSize: 12,
    ...
    >
      ...
      <div style={{ width: "100%", maxWidth: 400 }}>
        ...
        {groups.map((group) => (
          ...
          >
            <span className="mainBtnText" style={{ color: COLORS.primary }}>
              {group.name}
            </span>
          </button>
        ))}
      ...
      <div className="section">
        ...
        <div style={{ display: "flex", gap: 10, marginBottom: 20 }}>
          <button
            ...
            style={{
              ...
              color: gameMode === "doubles" ? "white" : "#666",
              fontWeight: "bold",
            }}
          >
            {t("modeDoubles")}
          ...
          <button
            ...
            style={{
              ...
              color: gameMode === "singles" ? "white" : "#666",
              fontWeight: "bold",
            }}
          >
            {t("modeSingles")}
        ...
        <div className="courtSelector">
          {[1, 2, 3, 4].map((num) => (
            ...
            >
              <span
                className={`courtBtnText ${numberOfCourts === num ? "active" : ""}`}
              >
                {num}
              </span>
            </button>
        ...
        <div className="courtSelector">
          {[11, 21].map((num) => (
            ...
            >
              <span
                className={`courtBtnText ${gameTarget === num ? "active" : ""}`}
              >
                {num}
              </span>
            </button>
      ...
      <div className="container">
        <div className="headerCompact">
          ...
          <div style={{ display: "flex", gap: 10 }}>
            ...
            >
              <span className="resetBtnText" style={{ color: COLORS.primary }}>
                {t("calcExpense")}
              </span>
            </button>
            <button
            ...
            >
              <span className="resetBtnText" style={{ color: COLORS.red }}>
                {t("resetGame")}
              </span>
            </button>
          </div>
        ...
        <div className="gameContent">
          ...
          >
            <h3 className="sectionHeader" style={{ margin: 0 }}>
              📊 {sortMode === "queue" ? t("queue") : t("leaderboard")}
            </h3>
            <button
              onClick={() =>
                setSortMode((prev) =>
                  prev === "queue" ? "leaderboard" : "queue",
                )
              }
              style={{
                background: "none",
            ...
            {players
              ...
              .map((p) => {
                ...
                return (
                  <div
                    key={p.id}
                    className={`statRow ${p.isPlaying ? "active" : ""}`}
                    style={{ opacity: p.isResting ? 0.6 : 1 }}
                  >
                    <button
                      onClick={() => {
                        setPlayers(
                      ...
                      style={{
                        ...
                        cursor: "pointer",
                        fontSize: 18,
                        filter: p.isResting ? "grayscale(100%)" : "none",
                        padding: 0,
                      }}
                    >
                      {p.isResting ? "💤" : "⚡"}
                    ...
                    >
                      <div
                        className="statusDot"
                        style={{
                          backgroundColor: p.isPlaying ? COLORS.accent : "#ddd",
                        }}
                      />
                      {p.photo && (
                        <img
                      ...
                      <span
                        className={`statName ${p.isPlaying ? "playing" : ""}`}
                      >
                        {p.name}
                        {p.isPlaying ? "(เล่น)" : ""}
          ...
          <div className="modalOverlay">
            <div className="modalView">
              ...
              <div className="modalActions">
                ...
                >
                  <span className="modalBtnTextCancel">
                    {t("cancelAction")}
                  </span>
                </button>
                <button
    ...
    <div className="app-root">
      ...
      {isShareModalVisible && (
        <div className="modalOverlay">
          <div className="modalView">
            ...
            <p style={{ fontSize: 13, color: COLORS.gray, marginBottom: 15 }}>
              {t("shareGroupDesc")}
            </p>
            <input
              className="modalInput"
        ...
        (() => {
          ...
          return (
            <div className="modalOverlay" onClick={closeCourtModal}>
              <div
                className="courtModalView"
                onClick={(e) => e.stopPropagation()}
              >
                <div className="courtModalHeader">
                  <h3 className="modalTitle">
                    🏸 {t("court")} {selectedCourtId}
                  </h3>
                  <button className="closeModalBtn" onClick={closeCourtModal}>
                    ✕
                  </button>
                </div>
                ...
                <div className="scoreboard">
                  <div className="scoreTeam">
                    ...
                    <div className="scoreControls">
                      <button
                        className="scoreBtn"
                        onClick={() => decrementScore(selectedCourtId, "team1")}
                      >
                        −
                      </button>
                      ...
                      <button
                        className="scoreBtn"
                        onClick={() => incrementScore(selectedCourtId, "team1")}
                      >
                        +
                      </button>
                    </div>
                  ...
                  <div className="scoreTeam">
                    ...
                    <div className="scoreControls">
                      <button
                        className="scoreBtn"
                        onClick={() => decrementScore(selectedCourtId, "team2")}
                      >
                        −
                      </button>
                      <div className="scoreDisplay">
                      ...
                      <button
                        className="scoreBtn"
                        onClick={() => incrementScore(selectedCourtId, "team2")}
                      >
                        +
                      </button>
                    </div>
                    ...
                    <div className="playerPositions">
                      {[0, 1].map((idx) => (
                        <div key={idx} className="playerSlot">
                          {team1Players[idx] ? (
                            <div
                              className={`playerCard ${team1Players[idx].photo ? "hasPhoto" : ""}`}
                            >
                              {team1Players[idx].photo && (
                                <img
                                  src={team1Players[idx].photo}
                                  className="playerPhotoMain"
                                  alt=""
                                />
                              )}
                              <button
                                ...
                                onClick={() =>
                                  removePlayerFromCourt(team1Players[idx].id)
                                }
                                style={{ zIndex: 2 }}
                              >
                                ×
                    ...
                    <div className="playerPositions">
                      {[0, 1].map((idx) => (
                        <div key={idx} className="playerSlot">
                          {team2Players[idx] ? (
                            <div
                              className={`playerCard ${team2Players[idx].photo ? "hasPhoto" : ""}`}
                            >
                              {team2Players[idx].photo && (
                                <img
                                  src={team2Players[idx].photo}
                                  className="playerPhotoMain"
                                  alt=""
                                />
                              )}
                              <button
                                className="removePlayerBtnSmall"
                                onClick={() =>
                                  removePlayerFromCourt(team2Players[idx].id)
                                }
                                style={{ zIndex: 2 }}
                              >
                                ×
                      ...
                      {availablePlayers.map((p) => (
                        <div key={p.id} className="availablePlayerItem">
                          <div className="availablePlayerInfo">
                            ...
                            <span className="availablePlayerCount">
                              {p.playCount} เกม
                            </span>
                          </div>
                          <button
        ...
        (() => {
          ...
          return (
            <div className="modalOverlay">
              ...
              >
                <h3 className="modalTitle">
                  {t("finishGameCourt")} {finishingCourtId}
                </h3>
                ...
                >
                  <button
                    ...
                    style={{
                      width: "100%",
                      backgroundColor:
                        suggestedWinner === "team1" ? "#006400" : "#f0f0f0",
                      color: suggestedWinner === "team1" ? "white" : "#333",
                    }}
                  ...
                  <button
                    ...
                    style={{
                      width: "100%",
                      backgroundColor:
                        suggestedWinner === "team2" ? "#006400" : "#f0f0f0",
                      color: suggestedWinner === "team2" ? "white" : "#333",
                    }}
      ...
      {isExpenseModalVisible &&
        (() => {
          ...
          const fixedIds = Object.keys(fixedPlayerFees).filter(
            (id) =>
              fixedPlayerFees[id] !== "" && fixedPlayerFees[id] !== undefined,
          );
          const totalFixed = fixedIds.reduce(
            (sum, id) => sum + (parseFloat(fixedPlayerFees[id]) || 0),
            0,
          );
          ...
          const splitShare =
            remainingPlayersCount > 0
              ? remainingExpense / remainingPlayersCount
              : 0;
                ...
                >
                  <table style={{ width: "100%", borderCollapse: "collapse" }}>
                    ...
                    <tbody>
                      {players.map((p) => {
                        const isFixed =
                          fixedPlayerFees[p.id] !== undefined &&
                          fixedPlayerFees[p.id] !== "";
                        const amount = isFixed
                          ? parseFloat(fixedPlayerFees[p.id])
                          : splitShare;
                ...
                <div style={{ borderTop: "1px solid #ddd", paddingTop: 15 }}>
                  ...
                  >
                    ...
                    <span style={{ color: "#006400" }}>
                      {Math.ceil(splitShare).toLocaleString()} {t("currency")}
                    </span>
                  </div>
                  <div
